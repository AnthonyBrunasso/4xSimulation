...
float EmpireUnits::operator()(uint32_t player_id, float threshold) {
  Player* player = player::get_player(player_id);
  if (!player) {
...
float EmpireCities::operator()(uint32_t player_id, float threshold) {
  Player* player = player::get_player(player_id);
  if (!player) {
...
EmpireUnits& empire_evaluations::get_units() {
  return s_units;
}

EmpireCities& empire_evaluations::get_cities() {
  return s_cities;
}
