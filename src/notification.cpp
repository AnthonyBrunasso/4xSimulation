...
namespace notification {
  NotificationVector get_player_notifications(uint32_t player_id) {
    ...
    if (player->m_research == SCIENCE_TYPE::UNKNOWN) {
      Notification n;
      n.m_id = 0;
      n.m_event_type = NOTIFICATION_TYPE::SCIENCE_IDLE;
    }
  ...
  std::string to_string(Notification& n) {
    std::stringstream ss;
    if (n.m_event_type == NOTIFICATION_TYPE::UNKNOWN) {
      ss << "Unknown notification." ;
    }
    else if (n.m_event_type == NOTIFICATION_TYPE::CITY_STARVING) {
    ...
    else if (n.m_event_type == NOTIFICATION_TYPE::CITY_PRODUCTION) {
      ss << "City (" << n.m_id << ") construction has idle_queue." ;
    }
    else if (n.m_event_type == NOTIFICATION_TYPE::CITY_SPECIALIZE) {
      ss << "City (" << n.m_id << ") has knowledge of the local terrain, specialize <cityId> <terrainType>." ;
    }
    else if (n.m_event_type == NOTIFICATION_TYPE::UNIT_IDLE) {
      ss << "Unit " << n.m_id << " is idle." ;
    }
    else if (n.m_event_type == NOTIFICATION_TYPE::SCIENCE_IDLE) {
