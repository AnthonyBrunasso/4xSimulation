...
void unit_definitions::initialize() {
  ...
  // Scout
  s_definitions[any_enum(fbs::UNIT_TYPE::SCOUT)] =
    CombatStats(3, 19.f, 11.f, 19.f, 1.f); 

  // Archer
  s_definitions[any_enum(fbs::UNIT_TYPE::ARCHER)] =
    CombatStats(2, 26.f, 13.f, 23.f, 2.f);

  // Phalanx
  s_definitions[any_enum(fbs::UNIT_TYPE::PHALANX)] =
    CombatStats(2, 48.f, 17.f, 37.f, 1.f);

  // Worker
  s_definitions[any_enum(fbs::UNIT_TYPE::WORKER)] = 
    CombatStats(2, 38.f, 0.f, 0.f, 0.f);

  // Wizard
  s_definitions[any_enum(fbs::UNIT_TYPE::WIZARD)] =
...
void unit_definitions::add(fbs::UNIT_TYPE id, const CombatStats& stats) {
  s_definitions[any_enum(id)] = stats;
}
...
void unit_definitions::for_each_definition(
  ...
  for (auto definition : s_definitions) {
    operation(any_enum(definition.first), definition.second);
  }
}
